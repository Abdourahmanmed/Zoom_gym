...
return <div className="min-h-screen lg:flex">
  ...
  <aside className="fixed inset-y-0 left-0 z-50 hidden w-64 border-r border-white/10 bg-card/90 p-4 backdrop-blur lg:block"><div className="mb-6 flex items-center gap-2 text-primary"><Dumbbell/> <span className="font-bold">EliteFit Djibouti</span></div>{navLinks}</aside>
  <aside className={`fixed inset-y-0 left-0 z-50 w-72 max-w-[85vw] border-r border-white/10 bg-card/95 p-4 backdrop-blur transition-transform duration-200 lg:hidden ${drawerOpen?"translate-x-0":"-translate-x-full"}`}><div className="mb-6 flex items-center justify-between gap-2 text-primary"><div className="flex items-center gap-2"><Dumbbell/> <span className="font-bold">EliteFit Djibouti</span></div><button onClick={()=>setDrawerOpen(false)} className="rounded-lg p-1 text-secondaryText hover:bg-white/10"><X size={18}/></button></div>{navLinks}</aside>
